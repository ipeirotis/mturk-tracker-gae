angular.module('mturk').controller('ArrivalsController', ['$scope', '$filter', 'dataService', 
    ...
    $scope.hitsChartHourly = {
            ...
            data: {"cols": [
                            ...
                            {id: "id2", label: "HITs arrived", type: "number"},],
                            "rows": []}
    };
    ...
    $scope.hitGroupsChartHourly = {
            ...
            data: {"cols": [
                            {id: "id1", label: "Date", type: "date"},
                            {id: "id2", label: "HIT groups arrived", type: "number"}],
                            "rows": []}
    };
    ...
    $scope.rewardsChartHourly = {
            ...
            data: {"cols": [
                            {id: "id1", label: "Date", type: "date"},
                            {id: "id2", label: "Rewards arrived", type: "number"}],
                            "rows": []}
    };
    ...
    $scope.load = function(){
        dataService.load($filter('date')($scope.from, 'MM/dd/yyyy'), $filter('date')($scope.to, 'MM/dd/yyyy'), function(response){ 
            ...
            angular.forEach(response.hourly, function(item){
                $scope.rows.hitsChartHourly.push({c:[{v: new Date(item.from)}, {v: parseInt(item.hitsArrived)}]});
                $scope.rows.hitGroupsChartHourly.push({c:[{v: new Date(item.from)}, {v: parseInt(item.hitGroupsArrived)}]});
                $scope.rows.rewardsChartHourly.push({c:[{v: new Date(item.from)}, {v: parseInt(item.rewardsArrived)/100}]});
            });

            angular.forEach(response.daily, function(item){
                $scope.rows.hitsChartDaily.push({c:[{v: new Date(item.from)}, {v: parseInt(item.hitsArrived)}]});
                $scope.rows.hitGroupsChartDaily.push({c:[{v: new Date(item.from)}, {v: parseInt(item.hitGroupsArrived)}]});
                $scope.rows.rewardsChartDaily.push({c:[{v: new Date(item.from)}, {v: parseInt(item.rewardsArrived)/100}]});
            });
